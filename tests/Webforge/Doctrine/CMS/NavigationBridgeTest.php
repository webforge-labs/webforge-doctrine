...
namespace Webforge\Doctrine\CMS;

use Webforge\TestData\NestedSet\FoodCategories;
use Webforge\CMS\Navigation\SimpleNode;
use Webforge\CMS\Navigation\NestedSetConverter; // in mock

class NavigationBridgeTest extends \Webforge\Code\Test\Base {
  ...
  public function setUp() {
    ...
    $this->food = new FoodCategories();
    $this->bridge = new NavigationBridge($this->em);
  ...
  protected function expectNestedSetConverterFromParentPointer() {
    list ($node1, $node2, $node3) = $this->nodes;
    $converter = $this->getMock('Webforge\CMS\Navigation\NestedSetConverter');
    $converter
